...
typealias TestCaseToResults = HashMap<String, MutableList<TestNgDataSetResult>>
...
class TestNgZephyrSuiteFactory(
    ...
    private fun toTestCasesWithResults(map: Map<String, MutableList<TestNgDataSetResult>>): List<TestCaseWithTestNgResults> {
        if (map.isEmpty()) {
            return emptyList()
        }
    ...
    private fun calculateTestStartEndTime(testNgDataSetResults: MutableList<TestNgDataSetResult>): Pair<Long, Long> {
        var startTime: Long
        var endTime: Long
