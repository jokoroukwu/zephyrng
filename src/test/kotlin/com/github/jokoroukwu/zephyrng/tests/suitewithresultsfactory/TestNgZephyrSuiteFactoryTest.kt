package com.github.jokoroukwu.zephyrng.tests.suitewithresultsfactory

import com.github.jokoroukwu.zephyrng.instantformatter.InstantToStringFormatterImpl
import com.github.jokoroukwu.zephyrng.suitewithresultsfactory.TestCaseResultsLinker
...
class TestNgZephyrSuiteFactoryTest {
    ...
    fun `test case should have proper start and end times`() {
        ...
        val dataSetResultMock = mockk<com.github.jokoroukwu.zephyrng.TestNgDataSetResult>(relaxed = true) {
            every { startTime } returns minStartTime
            every { endTime } returns maxEndTime - 1
        }
        val dataSetResultMockTwo = mockk<com.github.jokoroukwu.zephyrng.TestNgDataSetResult>(relaxed = true) {
    ...
    fun `test results should be grouped properly`() {
        ...
        val resultMap = testNgZephyrSuiteFactory.get(suiteMock)?.testCasesWithDataSetResults
            ?.groupBy(
                ...
                com.github.jokoroukwu.zephyrng.TestCaseWithTestNgResults::dataSetResults
            )
    ...
    fun `suite should have expected planned start and end dates`() {
        ...
        every { testCaseResultLinkerMock.mapTestCaseKeyToDataSetResult(any()) } returnsMany listOf(
            Pair(
                "1",
                com.github.jokoroukwu.zephyrng.TestNgDataSetResult(controlStartMillis, expectedEndMillis, 0, true)
            ),
            Pair(
                "2",
                com.github.jokoroukwu.zephyrng.TestNgDataSetResult(expectedStartMillis, controlEndMillis, 0, true)
            )
        )
