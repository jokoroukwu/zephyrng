package com.github.jokoroukwu.zephyrng.tests.testcaserequestsender
...
class TestCaseRequestSenderTest {
    ...
    fun `should throw expected exception when status code validation failed`() {
        val requestMock = mockk<AuthenticatedRequest>()
        mockkStatic("com.github.kittinunf.fuel.core.Deserializable")
        every { requestFactoryMock.get(any<String>()) } returns requestMock
        every { requestMock.authentication() } returns requestMock
        every { requestMock.basic(any(), any()) } returns requestMock
        every { requestMock.url } returns URL("http://test-url")
        justRun { requestMock.executionOptions.responseValidator = any() }
        every { requestMock.response(ZephyrResponseDeserializer) } returns mockk {
            every { third.get() } returns mockk(relaxed = true) {
                every { statusCode } returns 500 //    any error status code
            }
        }
        val result = requestMock.response(ZephyrResponseDeserializer)
        val opk = 1
        GetTestCasesRequestSender(
            zephyrConfig,
